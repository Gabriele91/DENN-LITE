...
namespace NRam 
{
	...
	bool DataSetTask::init(NRam::Task& task)
	{
		...
		m_train.m_meta["max_int"]   = std::make_shared<Variant>(std::get<4>(train));
		m_train.m_meta["timesteps"] = std::make_shared<Variant>(std::get<5>(train));
		...
		m_validation.m_meta["max_int"]   = std::make_shared<Variant>(std::get<4>(validation));
		m_validation.m_meta["timesteps"] = std::make_shared<Variant>(std::get<5>(validation));

		m_test.m_features 				 = std::get<0>(test);
		m_test.m_labels   				 = std::get<1>(test);
		m_test.m_mask     				 = std::get<2>(test);
		m_test.m_meta["max_int"]   = std::make_shared<Variant>(std::get<4>(test));
		m_test.m_meta["timesteps"] = std::make_shared<Variant>(std::get<5>(test));
	...
	bool DataSetTask::read_test(const DennAlgorithm& algorithm, DataSet& t_out) 
	{
		...
		m_test.m_labels   		 = std::get<1>(test);
		m_test.m_mask     		 = std::get<2>(test);
		m_test.m_meta["max_int"] = std::make_shared<Variant>(Variant(std::get<4>(test)));
		m_test.m_meta["timesteps"] = std::make_shared<Variant>(Variant(std::get<5>(test)));
	...
	bool DataSetTask::read_validation(const DennAlgorithm& algorithm, DataSet& t_out) 
	{
		...
		m_validation.m_labels   		= std::get<1>(validation);
		m_validation.m_mask 			  = std::get<2>(validation);
		m_validation.m_meta["max_int"] = std::make_shared<Variant>(Variant(std::get<4>(validation)));
		m_validation.m_meta["timesteps"] = std::make_shared<Variant>(Variant(std::get<5>(validation)));
	...
	bool DataSetTask::read_batch(const DennAlgorithm& algorithm, DataSet& t_out, bool loop) 
	{
		...
		m_train.m_labels   			= std::get<1>(train);
		m_train.m_mask     			= std::get<2>(train);
		m_train.m_meta["max_int"] = std::make_shared<Variant>(Variant(std::get<4>(train)));
		m_train.m_meta["timesteps"] = std::make_shared<Variant>(Variant(std::get<5>(train)));
