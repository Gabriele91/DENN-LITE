...
namespace NRamEnhanced
{
	...
	public:
		...
		mutable NRam::Task::SPtr			  m_task;	       //< nram task
		mutable NRam::DataSetTask           m_dataset;	   //< dataset
		...
		NRamInstance(const Denn::Parameters& parameters) : m_parameters(parameters)
		{
			...
			// gete list of gates
			std::vector< NRam::Gate::SPtr > gates;
			for (auto& gate_name : *(parameters.m_gates)) gates.push_back(NRam::GateFactory::create(gate_name));
			//init nram context
			m_nram.init
			(
        ...
        , *parameters.m_nram_enhanced
			...
			m_dataset = *m_task;
			//network
			m_network = build_mlp_network(m_nram.m_n_regs + 1 /* timestep */, m_nram.m_nn_output, parameters);
	...
	REGISTERED_INSTANCE(NRamInstance, "nram_enhanced")
}
}
