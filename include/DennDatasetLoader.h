...
namespace Denn
{
	...
	protected:
		...
		bool template_read(DataSetX<ScalarType>& t_out, const unsigned int samples, const unsigned int depth)
		{
			...
			//depth
			t_out.features<ScalarType>().resize(m_header.m_n_features*depth, samples);
			//levels
			...
			for (unsigned int d = 0; d != depth; ++d)
			{
				...
				//append
				t_out.features<ScalarType>().block(m_header.m_n_features*d, 0, m_header.m_n_features*(d+1), samples) = alevel;
			}
			//alloc output
			//data are in row-major layout then the shape is traspose
			t_out.labels<ScalarType>().resize(m_header.m_n_classes, samples);
			//read labels
